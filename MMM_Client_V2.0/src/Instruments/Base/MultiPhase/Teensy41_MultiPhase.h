...
using std::int8_t;
    ...
    #define INSTRUMENT_TYPE Teensy41_MultiPhase
#endif

/* Outputs a PWM signal at the Notes Frequency on each Digital IO Pin */
class Teensy41_MultiPhase : public InstrumentControllerBase{
protected:
    static void tick();
    static void updatePhase(uint8_t instrument);
    ...
    static std::array<uint8_t,HardwareConfig::MAX_NUM_INSTRUMENTS> m_currentState; //Step in Wave Table
